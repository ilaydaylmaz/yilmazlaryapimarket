...
// Showroom fotoğraflarını getir (Admin)
...
// Showroom fotoğraflarını kaydet (Admin)
app.post("/api/showroom", auth, upload.array("images", 10), async (req, res) => {
  try {
    let images = [];
    ...
    if (req.files && req.files.length > 0) {
      req.files.forEach(file => {
        images.push({
          ...
          title: req.body.titles ? JSON.parse(req.body.titles)[file.filename] || '' : '',
          description: req.body.descriptions ? JSON.parse(req.body.descriptions)[file.filename] || '' : '',
          order: images.length,
    ...
    res.json({ success: true, message: "Showroom fotoğrafları kaydedildi", data: data });
  } catch (error) {
...
// Showroom fotoğrafını güncelle (Admin)
app.put("/api/showroom/:id", auth, (req, res) => {
  try {
    if (!fs.existsSync(SHOWROOM_FILE)) {
      return res.status(404).json({ success: false, message: "Showroom bulunamadı" });
    }
    ...
    // Güncelle
    if (req.body.title !== undefined) data.images[imageIndex].title = req.body.title;
    if (req.body.description !== undefined) data.images[imageIndex].description = req.body.description;
    if (req.body.order !== undefined) data.images[imageIndex].order = parseInt(req.body.order);
    
    data.updatedAt = new Date().toISOString();
    fs.writeFileSync(SHOWROOM_FILE, JSON.stringify(data, null, 2), 'utf8');
    res.json({ success: true, message: "Fotoğraf güncellendi", data: data.images[imageIndex] });
  } catch (error) {
    console.error("Showroom güncelleme hatası:", error);
    res.status(500).json({ success: false, message: "Sunucu hatası: " + error.message });
  }
});

// Showroom fotoğrafını sil (Admin)
app.delete("/api/showroom/:id", auth, (req, res) => {
  try {
    if (!fs.existsSync(SHOWROOM_FILE)) {
      return res.status(404).json({ success: false, message: "Showroom bulunamadı" });
    }
    ...
    console.error("Showroom silme hatası:", error);
    res.status(500).json({ success: false, message: "Sunucu hatası: " + error.message });
  }
});

// Public showroom fotoğrafları
app.get("/api/public/showroom", (req, res) => {
  try {
    if (fs.existsSync(SHOWROOM_FILE)) {
      const data = JSON.parse(fs.readFileSync(SHOWROOM_FILE, 'utf8'));
